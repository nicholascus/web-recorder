...
export class TwitterFeed extends AbstractWebParser<Tweet> {
    ...
    async extractUid(page: Page, element: Locator): Promise<string> {
        // tweet id
        const tweetIdElement = element
            .locator('xpath=/descendant::a[contains(@href,"status")]')
            .first();
        const tweetHrefId = await tweetIdElement.getAttribute('href');
        const tweetIdMatch = tweetHrefId.match(
            /\/.*\/status\/(\d*)(\/analytics|)/,
        );
    ...
    async extractFullRecord(page: Page, element: Locator): Promise<Tweet> {
        ...
        const parentIdMatch = url.pathname.match(
            /\/(.*)\/status\/(\d*)(\/analytics|)/,
        );
        if (parentIdMatch) {
        ...
        // tweet or ad ?
        tweet.isAd = !!(await element
            .locator('xpath=/descendant::span[.="Ad"]')
            .count());

        if (!tweet.isAd) {
            tweet.time = await element
                .locator('xpath=/descendant::time')
                .first()
                .getAttribute('datetime');
        }

        tweet.hasQuote = !!(await element
            .locator('xpath=/descendant::span[.="Quote"]')
            .count());
        if (tweet.hasQuote) {
            tweet.quote = await this.extractFullRecord(
                page,
                element
                    .locator(
                        'xpath=/descendant::div[div/span[.="Quote"]]/div[descendant::*[@data-testid="tweetText"]]',
                    )
                    .first(),
            );
        }

        tweet.content = await element
            .getByTestId('tweetText')
            .first()
            .innerText();
...
export class TweetConsoleLogger implements IContentWriter<Tweet> {
    log(t: Tweet): void {
        console.log(
            `\n!FOUND_NEW_TWEET! uid=${t.uid}, isAd=${
                t.isAd ? 'Y' : 'N'
            }, time=${t.time}, user=${t.username}${
                t.hasQuote
